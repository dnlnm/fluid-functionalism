...
const Checkbox = forwardRef<HTMLDivElement, CheckboxProps>(
  (
    ...
    const emit = (next: boolean | "indeterminate") => {
      if (disabled) return;
      // Base UI passes (checked, eventDetails); narrow to a boolean.
      const resolved = next === true;
      onCheckedChange?.(resolved);
      onToggle?.();
    };

    const markPath = checked === "indeterminate" ? "M6 12H18" : "M6 12L10 16L18 8";
    const markKey = checked === "indeterminate" ? "dash" : "check";

    const box = (
      <CheckboxPrimitive.Root
        checked={checked === true}
        indeterminate={checked === "indeterminate"}
        // Base UI passes (checked, eventDetails); narrow before invoking ours.
        onCheckedChange={(next) => emit(next)}
        disabled={disabled}
